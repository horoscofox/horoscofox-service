...
def test_no_sign_no_kind():
    ...
    assert response.json() == {
        "message": "ERR0R: Sorry but branko is away at the moment"}

    response = client.get('/mymom')
    assert response.status_code == 400
    assert response.json() == {
        "message": "ERR0R: No astrologer accepted!"}
...
def test_no_kind():
    ...
    assert response.status_code == 400
    assert response.json() == {
        "message": "ERR0R: Sorry but there is currently no horoscope of the virgo readed by paolo"}
...
def test_no_astrologer_or_sign():
    ...
    assert response.status_code == 400
    assert response.json() == {
        "message": "ERR0R: vergine is not valid"}
